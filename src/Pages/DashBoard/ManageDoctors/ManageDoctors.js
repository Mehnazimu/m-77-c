...
const ManageDoctors = () => {
    ...
    const { data: doctors, isLoading ,refetch} = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await fetch('https://m-77-server.vercel.app/doctors', {
                    headers: {
    ...
    const handleDoctorDelete= doctor =>{
       fetch(`https://m-77-server.vercel.app/doctors/${doctor._id}`,{
        method: 'DELETE',
        headers:{
            authorization: `bearer ${localStorage.getItem('accessToken')}`
        }
    ...
    return (
        <div>
            ...
            </div>
            {
                ...
                message={`If you delete ${deletingDoctor.name},It cannot br undone`}
